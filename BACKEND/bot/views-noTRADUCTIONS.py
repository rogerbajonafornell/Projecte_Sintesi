...
def process_update(body):
    ...
    if action == 'search' and article:
        ...
        if isinstance(art_obj, Article):
            ...
            # Assegura't que art_obj és iterable abans de convertir-lo en llista
            if hasattr(art_obj, '__iter__'):
                ...
                if not similar_articles:
                    ...
                    send_telegram_message(chat_id, error_message)
                    return
                options = [f"·🛍️{chr(97+i)}) {a.DescripcionArticulo}\n" for i, a in enumerate(similar_articles)]
                ...
                print(f"Error: art_obj no és iterable. Tipus: {type(art_obj)}")
